...
class Zlibrary:
    ...
    def loginWithToken(self, remix_userid: [int, str],
                       remix_userkey: str) -> dict[str, str]:
        return self.__checkIDandKey(remix_userid, remix_userkey)

    def __makePostRequest(self,
                          url: str,
                          data: dict = {},
                          override=False) -> dict[str, str]:
    ...
    def __makeGetRequest(self,
                         url: str,
                         params: dict = {},
                         cookies=None) -> dict[str, str]:
    ...
    def recoverPassword(self, email: str) -> dict[str, str]:
        return self.__makePostRequest("/eapi/user/password-recovery",
                                      {"email": email},
                                      override=True)

    def makeRegistration(self, email: str, password: str,
    ...
    def sendTo(self, bookid: [int, str], hashid: str,
               totype: str) -> dict[str, str]:
        return self.__makeGetRequest(
            f"/eapi/book/{bookid}/{hashid}/send-to-{totype}")

    def getBookInfo(self,
                    bookid: [int, str],
                    hashid: str,
                    switch_language: str = None) -> dict[str, str]:
        if switch_language is not None:
    ...
    def __getBookFile(self, bookid: [int, str],
                      hashid: str) -> [(str, bytes), None]:
        response = self.__makeGetRequest(f"/eapi/book/{bookid}/{hashid}/file")
        file_info = response.get("file", {})
        filename = (file_info.get("description") or file_info.get("name") or
                    file_info.get("title") or f"zlibrary-{bookid}")
        ...
        if not file_info or "downloadLink" not in file_info:
            err_msg = response.get("message") if response else "Unknown Z-Library EAPI error"
            raise RuntimeError(f"Download failed: {err_msg} (EAPI response: {response})")
        ddl = file_info["downloadLink"]
    ...
    def verifyCode(self, email: str, password: str, name: str,
