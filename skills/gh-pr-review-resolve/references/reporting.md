...
List fixed threads first, then still-unresolved and needs-decision threads by priority. When recency matters, summarize the whole thread while using its latest comment to determine ordering.
...
Draft replies only when the user chose the reply-drafting option or explicitly requested them.
